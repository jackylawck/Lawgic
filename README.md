...
* **珍珠迴路 (`masyu`)**：**四大古典宏觀範式（螺線/編織/曲流/鞍點）**、HMC 蒙特卡羅退火生成、CNN 幾何潛在美學濾波、真 L1~L5 時態光暈反證樹、**原子化筆劃事務提交（零時間脫鉤）**、轉折點認知書籤、失焦物理微光護盾與賽後拓撲譜系指紋。
...
| --- | --- | --- | --- |
| `masyu` | **珍珠迴路** | 空間拓撲、歐拉迴路、反證假說 | **古典幾何範式（螺線/編織/曲流/鞍點）、先驗作者意圖圖譜、HMC 退火採樣、輕量 CNN 潛在美學濾波（太極/海馬/四葉）、極限稀疏 SAT 剪枝（密度 $\le 12\%$）、真 L1~L5 矛盾樹、原子筆劃事務管線、時間線小地圖、創世紀退火重播劇場** |
...
* **Exact Cover Uniqueness Engine**: Mathematical certainty replacing heuristic timeouts. Across all 18 puzzle variants, exact MRV backtracking solvers, SAT pruners, and bipartite matching proofs guarantee puzzle solution cardinality equals exactly 1.
...
* **Masyu**: Macro-paradigms (spiral, braid, meander, cross), HMC annealed loop generation, latent CNN aesthetic motif filter, true L1–L5 tense-aware contradiction trees, **atomic stroke mutations (zero temporal tearing)**, directional-turn bookmarks, and spatial defocus field.
...
flowchart TD
    ...
    Gv --> M2["Masyu / Slitherlink (拓撲封閉迴路 / HMC退火)"]
    Gv --> M3["Nurikabe (平面分割)"]
    ...
    Gwm --> W2["時態深層前瞻反證樹沙盒 (Tense-Aware Lookahead)"]
    Gwm --> W3["錯誤類型學、時間線小地圖與即時 APM 時序監測"]
...
│  • Compile-time PEERS LUT    │ │ • HMC Annealed Masyu Engine│
│  • Bitmask MRV Propagation   │ │ • Full Kropki Box Topology │
...
│   ├── src/
│   │   ├── components/      # 18 款競速駕駛艙 (Board)、因果高亮、時間線小地圖與互動元件
│   │   ├── contexts/        # 語系切換 (LanguageContext) 與無障礙支援 (AccessibilityContext)
│   │   ├── engines/         # 18 款競技級演算法 (Masyu Apex, Kropki Apex, Kakuro Perfect 等)
...
│   │   ├── workers/         # 多執行緒背景生產守護池 (masyu.worker.ts 等)
