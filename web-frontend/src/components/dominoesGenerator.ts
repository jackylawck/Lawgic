...
export type DominoBorderState = 0 | 1 | 2; // 0: 未決, 1: 骨牌連線, 2: 隔離牆
      ...
      for (let c = 0; c < cols; c++) {
        ...
        if (openNeighbors.length === 1) {
          ...
          return {
            ...
            rationale: `格 [${r + 1},${c + 1}] 僅剩 [${nr + 1},${nc + 1}] 為唯一可連鄰居，強制成牌。`,
            humanReadable: {
              zh: `觀察 [${r + 1}, ${c + 1}]：其他方向皆被隔離牆阻斷，只能與 [${nr + 1}, ${nc + 1}] 結合成牌。`,
    ...
    for (const piece of dominoSet) {
      ...
      if (candidates.length === 1) {
        ...
        return {
          ...
          rationale: `骨牌 [${piece.val1}-${piece.val2}] 全盤僅剩此唯一合法位置，強制成牌。`,
          humanReadable: {
            zh: `骨牌 [${piece.val1}-${piece.val2}] 僅存 [${r1 + 1},${c1 + 1}] 與 [${r2 + 1},${c2 + 1}] 能容納，必然成牌。`,
